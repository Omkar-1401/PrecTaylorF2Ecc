...
DEFINE_GENERATOR_TEMPLATE(PrecTaylorF2Ecc, NULL, generate_fd_waveform, NULL, NULL)
...
)
{
    ...
    case PrecTaylorF2Ecc:
        ...
        /* Call the waveform driver routine */
        ret = XLALSimInspiralPrecTaylorF2Ecc(hptilde, phiRef, deltaF, m1, m2, S1z, S2z, S1x, S2x, S1y, S2y, f_min, f_max, f_ref, distance, eccentricity, params);
        if (ret == XLAL_FAILURE)
            XLAL_ERROR(XLAL_EFUNC);
        /* Produce both polarizations */
        *hctilde = XLALCreateCOMPLEX16FrequencySeries("FD hcross", &((*hptilde)->epoch), (*hptilde)->f0, (*hptilde)->deltaF, &((*hptilde)->sampleUnits), (*hptilde)->data->length);
